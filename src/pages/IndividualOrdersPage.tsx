...
export default function IndividualOrdersPage() {
  const { user, profile, signOut } = useIndividual();
  ...
  const filteredBookings = useMemo(() => {
    let list = bookings.filter(b => {
      if (search) {
        ...
        if (!b.booking_ref.toLowerCase().includes(q) && !b.sender_name.toLowerCase().includes(q) &&
            !b.recipient_name.toLowerCase().includes(q) && !b.pickup_city.toLowerCase().includes(q) &&
  ...
  const filteredRequests = useMemo(() => {
    let list = requests.filter(r => {
      if (search) {
        const q = search.toLowerCase();
        if (!r.request_ref.toLowerCase().includes(q) && !r.title.toLowerCase().includes(q) &&
            !r.origin.toLowerCase().includes(q) && !r.destination.toLowerCase().includes(q)) return false;
      }
          ...
          <div className="p-4 border-b border-white/10">
            <div className="bg-white/5 rounded-xl p-4">
              ...
              <span className="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full bg-green-500/20 text-green-400">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400" /> Individual
              </span>
            </div>
          </div>
          <nav className="flex-1 p-4 space-y-1">
            <p className="text-gray-500 text-xs font-bold uppercase tracking-wider px-3 mb-2">Navigation</p>
            <Link to="/individual/dashboard" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-300 hover:bg-white/10 hover:text-white transition-all text-sm">
              <BarChart3 className="h-4 w-4" /> Dashboard
            </Link>
            <Link to="/individual/orders" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10 text-white font-medium text-sm">
            ...
            <Link to="/individual/messages" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-300 hover:bg-white/10 hover:text-white transition-all text-sm">
        ...
        <main className="flex-1 lg:ml-64">
          <header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-20">
            <div className="flex items-center gap-3">
              <Link to="/individual/dashboard" className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors lg:hidden">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              ...
              <div className="px-5 py-3 border-b border-gray-50 flex flex-wrap gap-2 items-center">
                ...
                <button onClick={() => setShowFilters(v => !v)}
                  ...
                  <Filter className="h-4 w-4" /> Filters {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />}
                </button>
                {hasActiveFilters && (
                ...
                <div className="px-5 py-3 bg-gray-50/60 border-b border-gray-100 flex flex-wrap gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                      ...
                      {(tab === 'bookings' ? BOOKING_STATUSES : REQUEST_STATUSES).map(s => <option key={s} value={s}>{cap(s)}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{tab === 'bookings' ? 'Package Type' : 'Service Type'}</label>
                    <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
                      className="text-sm border border-gray-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 bg-white">
                      <option value="">All types</option>
                      {(tab === 'bookings' ? PACKAGE_TYPES : SERVICE_TYPES).map(t => <option key={t} value={t}>{cap(t)}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1">
                ...
                <>
                      ...
                      {filteredBookings.length === 0 ? (
                        ...
                        <table className="w-full text-sm">
                          <thead className="bg-gray-50/80 border-b border-gray-100">
                            <tr>
                              <th className={thClass} onClick={() => handleSort('ref')}><span className="flex items-center gap-1">Ref <SortIcon field="ref" active={sortField} dir={sortDir} /></span></th>
                              ...
                              <th className={thClass} onClick={() => handleSort('destination')}><span className="flex items-center gap-1">Route <SortIcon field="destination" active={sortField} dir={sortDir} /></span></th>
                              <th className={thClass} onClick={() => handleSort('type')}><span className="flex items-center gap-1">Type <SortIcon field="type" active={sortField} dir={sortDir} /></span></th>
                              <th className={thClass} onClick={() => handleSort('status')}><span className="flex items-center gap-1">Status <SortIcon field="status" active={sortField} dir={sortDir} /></span></th>
                              <th className={thClass} onClick={() => handleSort('created_at')}><span className="flex items-center gap-1">Date <SortIcon field="created_at" active={sortField} dir={sortDir} /></span></th>
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {(pageSlice as Booking[]).map(b => (
                              <tr key={b.id} className="hover:bg-orange-50/30 transition-colors">
                                <td className="px-4 py-3.5"><span className="font-mono font-semibold text-gray-900 text-xs">{b.booking_ref}</span></td>
                                <td className="px-4 py-3.5"><p className="font-medium text-gray-900 truncate max-w-28">{b.sender_name}</p><p className="text-xs text-gray-400 truncate max-w-28">{b.pickup_city}</p></td>
                                <td className="px-4 py-3.5"><p className="font-medium text-gray-900 truncate max-w-28">{b.recipient_name}</p><p className="text-xs text-gray-400 truncate max-w-28">{b.recipient_phone}</p></td>
                                <td className="px-4 py-3.5"><p className="text-gray-700 text-xs">{b.pickup_city}</p><p className="text-gray-400 text-xs">→ {b.delivery_city}</p></td>
                                <td className="px-4 py-3.5"><span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md font-medium">{cap(b.package_type)}</span></td>
                                <td className="px-4 py-3.5"><span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${BOOKING_STATUS_STYLES[b.status] ?? 'bg-gray-50 text-gray-600 border-gray-200'}`}>{cap(b.status)}</span></td>
                                <td className="px-4 py-3.5"><div className="flex items-center gap-1 text-gray-500 text-xs"><Calendar className="h-3 w-3" />{new Date(b.created_at).toLocaleDateString()}</div></td>
                                <td className="px-4 py-3.5 text-right">
                                  <div className="flex items-center justify-end gap-1.5">
                                    <button onClick={() => setSelectedBooking(b)} className="p-1.5 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="View details"><Eye className="h-3.5 w-3.5" /></button>
                                    <button onClick={() => setInvoiceData({ type: 'booking', ...b, agent_company: profile?.full_name, agent_name: profile?.full_name, agent_phone: profile?.phone, agent_email: profile?.email })} className="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors" title="Generate invoice"><FileDown className="h-3.5 w-3.5" /></button>
                                  </div>
                                </td>
                              </tr>
                            ))}
                        ...
                        <table className="w-full text-sm">
                          <thead className="bg-gray-50/80 border-b border-gray-100">
                            <tr>
                              ...
                              <th className={thClass} onClick={() => handleSort('type')}><span className="flex items-center gap-1">Service <SortIcon field="type" active={sortField} dir={sortDir} /></span></th>
                              <th className={thClass} onClick={() => handleSort('destination')}><span className="flex items-center gap-1">Route <SortIcon field="destination" active={sortField} dir={sortDir} /></span></th>
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {(pageSlice as LogisticsRequest[]).map(r => (
                              <tr key={r.id} className="hover:bg-blue-50/20 transition-colors">
                                ...
                                <td className="px-4 py-3.5"><div className="flex items-center gap-1 text-gray-500 text-xs"><Calendar className="h-3 w-3" />{new Date(r.created_at).toLocaleDateString()}</div></td>
                                <td className="px-4 py-3.5 text-right">
                                  <button onClick={() => setInvoiceData({ type: 'request', ...r, agent_company: profile?.full_name, agent_name: profile?.full_name, agent_phone: profile?.phone, agent_email: profile?.email })} className="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors" title="Generate invoice"><FileDown className="h-3.5 w-3.5" /></button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                  ...
                  {activeList.length > PAGE_SIZE && (
                    <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
                      <p className="text-xs text-gray-500">Showing {((page - 1) * PAGE_SIZE) + 1}–{Math.min(page * PAGE_SIZE, activeList.length)} of {activeList.length}</p>
                      <div className="flex items-center gap-1">
                        <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors">Prev</button>
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                          const pg = Math.max(1, Math.min(page - 2, totalPages - 4)) + i;
                          return <button key={pg} onClick={() => setPage(pg)} className={`w-8 h-8 text-xs rounded-lg transition-colors ${pg === page ? 'bg-orange-500 text-white font-semibold' : 'border border-gray-200 hover:bg-gray-50 text-gray-600'}`}>{pg}</button>;
                        })}
                        <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors">Next</button>
                      </div>
                    </div>
                  )}
                </>
