    ...
    <div className="flex flex-col h-full bg-white overflow-hidden md:rounded-2xl md:border md:border-gray-100">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
        {loading ? (
          ...
          <button key={t.id} onClick={() => { setActiveThread(t); setMobileView('thread'); }}
            ...
            <div className="flex items-start gap-3">
              ...
              {(t.unread_count || 0) > 0 && (
                <span className="w-5 h-5 bg-orange-500 text-white text-xs rounded-full flex items-center justify-center font-bold flex-shrink-0">{t.unread_count}</span>
              )}
            </div>
          </button>
        ))}
                ...
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] sm:max-w-[70%] flex flex-col gap-1 ${isMe ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                      isMe ? 'bg-gradient-to-br from-orange-500 to-red-500 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'
                    }`}>{msg.body}</div>
          ...
          <div className="px-3 py-3 border-t border-gray-100">
            <div className="flex items-end gap-2">
              <textarea value={reply} onChange={e => setReply(e.target.value)}
                ...
                placeholder="Reply to admin..." rows={2}
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          <Link to="/rider/dashboard" className="p-2 hover:bg-gray-100 rounded-xl transition-colors text-gray-600">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          ...
          <button onClick={handleLogout}
            className="ml-auto p-2 hover:bg-red-50 text-gray-400 hover:text-red-500 rounded-xl transition-colors" title="Sign out">
