...
const SERVICE_COLORS: Record<string, string> = {
  freight: 'bg-blue-500', warehousing: 'bg-green-500',
  express: 'bg-yellow-500', bulk: 'bg-orange-500',
  customs: 'bg-teal-500', last_mile: 'bg-red-500',
};

const PACKAGE_COLORS: Record<string, string> = {
  document: 'bg-blue-400', parcel: 'bg-orange-400',
  fragile: 'bg-yellow-400', heavy: 'bg-slate-500',
};
...
function DonutChart({ segments }: { segments: { label: string; value: number; color: string }[] }) {
  ...
  const colorMap: Record<string, string> = {
    'bg-blue-400': '#60a5fa', 'bg-orange-400': '#fb923c', 'bg-yellow-400': '#facc15',
    'bg-slate-500': '#64748b', 'bg-blue-500': '#3b82f6', 'bg-green-500': '#22c55e',
    'bg-yellow-500': '#eab308', 'bg-orange-500': '#f97316', 'bg-red-500': '#ef4444',
    'bg-teal-500': '#14b8a6',
  };
  return (
      ...
      {segments.map((seg, i) => {
        ...
        const el = (
          <circle key={i} cx="50" cy="50" r={r} fill="none"
            stroke={colorMap[seg.color] ?? '#94a3b8'} strokeWidth="18"
...
export default function BusinessDashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchUnread = async () => {
      const { data: threads } = await supabase.from('message_threads').select('id').eq('recipient_id', user.id).eq('recipient_type', 'business');
  ...
  const fetchData = async () => {
    ...
    const [bRes, rRes, recentBRes, recentRRes] = await Promise.all([
      ...
      supabase.from('business_logistics_requests').select('id, request_ref, title, description, service_type, vehicle_type, origin, destination, quantity, weight, preferred_date, budget_range, status, created_at').eq('business_id', user.id).order('created_at', { ascending: false }).limit(6),
    ]);

    const bAll = bRes.data ?? [];
    const rAll = rRes.data ?? [];
    ...
    setAnalytics({
      totalBookings: bAll.length, bookingsThisMonth: bThisMonth, bookingsLastMonth: bLastMonth,
      pendingBookings: bByStatus.pending ?? 0, confirmedBookings: bByStatus.confirmed ?? 0,
      inTransitBookings: (bByStatus.in_transit ?? 0) + (bByStatus.picked_up ?? 0),
      deliveredBookings: delivered, cancelledBookings: cancelled,
      deliveryRate: pct(delivered, activable), cancellationRate: pct(cancelled, bAll.length),
      totalRequests: rAll.length, requestsThisMonth: rThisMonth, requestsLastMonth: rLastMonth,
      pendingRequests: rByStatus.pending ?? 0, approvedRequests: (rByStatus.approved ?? 0) + (rByStatus.in_progress ?? 0),
      completedRequests: completed, rejectedRequests: rejected,
      requestSuccessRate: pct(completed, rClosed),
      packageTypeCounts: pkgCounts, serviceTypeCounts: svcCounts,
    });
  ...
  const pkgSegments = Object.entries(analytics.packageTypeCounts).map(([k, v]) => ({ label: cap(k), value: v, color: PACKAGE_COLORS[k] ?? 'bg-slate-400' }));
  const svcSegments = Object.entries(analytics.serviceTypeCounts).map(([k, v]) => ({ label: cap(k), value: v, color: SERVICE_COLORS[k] ?? 'bg-slate-400' }));
      ...
      <div className="flex min-h-screen">
        ...
        <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full z-30 hidden lg:flex">
            ...
            <div className="bg-white/5 rounded-xl p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-gray-400 text-xs truncate">{profile.industry}</p>
                </div>
              </div>
          ...
          <nav className="flex-1 p-4 space-y-1">
            ...
            <Link to="/business/dashboard" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10 text-white font-medium text-sm">
          ...
          <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            {profile.status === 'pending' && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-6 flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold text-yellow-800 text-lg mb-1">Business Application Under Review</h3>
                  <p className="text-yellow-700 text-sm leading-relaxed">
                    Your business registration is pending admin approval. We'll notify you by email once your account is activated. This usually takes 24–48 hours.
                  </p>
                  <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-yellow-600">
            ...
            {profile.status === 'rejected' && (
              <div className="bg-red-50 border border-red-200 rounded-2xl p-6 flex items-start gap-4">
                ...
                <div>
                  ...
                  {profile.rejection_reason && <p className="text-red-700 text-sm mb-2">Reason: {profile.rejection_reason}</p>}
            ...
            {profile.status === 'approved' && (
              <>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        ...
                        analytics.deliveryRate >= 50 ? 'bg-yellow-50 text-yellow-600' : 'bg-red-50 text-red-600'
                      }`}>
                        {analytics.deliveryRate >= 80 ? 'Excellent' : analytics.deliveryRate >= 50 ? 'Good' : 'Needs Work'}
                      </span>
                    </div>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        ...
                        analytics.requestSuccessRate >= 50 ? 'bg-yellow-50 text-yellow-600' : 'bg-gray-50 text-gray-500'
                      }`}>
                ...
                {/* KPI Row 2: Booking Pipeline + Cancellation Gauge */}
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-6 flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center gap-3">
                      ...
                      <div className={`text-xs font-semibold px-3 py-1.5 rounded-full ${
                        ...
                        analytics.cancellationRate > 10 ? 'bg-orange-50 text-orange-600' : 'bg-green-50 text-green-600'
                      }`}>
                        {analytics.cancellationRate > 20 ? 'High — needs attention' : analytics.cancellationRate > 10 ? 'Moderate' : 'Low — great!'}
                      </div>
                    </div>
                  </div>
                </div>

                {/* KPI Row 3: Breakdowns */}
                <div className="grid lg:grid-cols-3 gap-6">
                  <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    {pkgSegments.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-6 text-gray-300">
                        <Package className="h-10 w-10 mb-2" /><p className="text-sm">No data yet</p>
                      </div>
                    ) : (
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    {svcSegments.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-6 text-gray-300">
                        <Truck className="h-10 w-10 mb-2" /><p className="text-sm">No data yet</p>
                      </div>
                    ) : (
                      <div className="flex items-center gap-5">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <Link to="/business/logistics/new"
                    ...
                    <p className="text-gray-300 text-sm">Freight, warehousing, bulk, express and more</p>
                  </Link>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                <Building2 className="h-5 w-5 text-gray-400" /> Company Profile
              </h3>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                {[
                  { icon: Building2, label: 'Company', value: profile.company_name },
                  { icon: Users, label: 'Contact', value: profile.contact_person },
                  { icon: Mail, label: 'Email', value: profile.email },
                  { icon: Phone, label: 'Phone', value: profile.phone },
                  { icon: Globe, label: 'Industry', value: profile.industry },
                  { icon: MapPin, label: 'City', value: profile.city },
                  { icon: Briefcase, label: 'Company Size', value: cap(profile.company_size) },
                  { icon: AlertTriangle, label: 'CAC / Reg No.', value: profile.registration_number },
                  { icon: TrendingUp, label: 'TIN', value: profile.tax_id || '—' },
                ].map(item => (
