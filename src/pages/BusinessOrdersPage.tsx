...
export default function BusinessOrdersPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`biz-bookings-rt-${user.id}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'business_delivery_bookings', filter: `business_id=eq.${user.id}` },
        (payload) => {
      ...
      <div className="flex min-h-screen">
        ...
        <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full z-30 hidden lg:flex">
          ...
          <nav className="flex-1 p-4 space-y-1">
            ...
            <Link to="/business/dashboard" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-300 hover:bg-white/10 hover:text-white transition-all text-sm">
              <BarChart3 className="h-4 w-4" /> Dashboard
            </Link>
            {profile.status === 'approved' && (
              <>
                <Link to="/business/orders" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10 text-white font-medium text-sm">
        ...
        <main className="flex-1 lg:ml-64">
          ...
          <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-20">
            <div className="flex items-center gap-3">
              <Link to="/business/dashboard" className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors lg:hidden">
                              ...
                              <tr key={b.id} className="hover:bg-orange-50/30 transition-colors group">
                                ...
                                <td className="px-4 py-3.5 text-right">
                                  <div className="flex items-center justify-end gap-1.5">
                                    ...
                                    <button
                                      onClick={() => setInvoiceData({ type: 'booking', ...b, business_name: profile?.company_name, business_contact: profile?.contact_person, business_phone: profile?.phone, business_email: profile?.email })}
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {(pageSlice as LogisticsRequest[]).map(r => (
                              <tr key={r.id} className="hover:bg-blue-50/20 transition-colors group">
                                ...
                                <td className="px-4 py-3.5 text-right">
                                  <button
                                    onClick={() => setInvoiceData({ type: 'request', ...r, business_name: profile?.company_name, business_contact: profile?.contact_person, business_phone: profile?.phone, business_email: profile?.email })}
                                    className="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors"
                                    title="Generate invoice"
                                  >
                                    <FileDown className="h-3.5 w-3.5" />
