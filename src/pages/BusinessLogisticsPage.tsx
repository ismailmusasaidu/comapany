    ...
    try {
      ...
      const { error: err } = await supabase.from('business_logistics_requests').insert({
        ...
        description: [
          ...
          customsData.companyName ? `Company: ${customsData.companyName}` : '',
          customsData.rcNumber ? `RC: ${customsData.rcNumber}` : '',
          customsData.tin ? `TIN: ${customsData.tin}` : '',
          customsData.additionalNotes ? `Notes: ${customsData.additionalNotes}` : '',
        ].filter(Boolean).join('\n'),
        origin: customsData.countryOrigin || '',
        destination: customsData.countryDestination || '',
        quantity: String(customsData.cargoItems.reduce((s, c) => s + (parseInt(c.quantity) || 0), 0)),
        weight: `${customsData.cargoItems.reduce((s, c) => s + (parseFloat(c.weightKg) || 0), 0).toFixed(1)} kg`,
    ...
    try {
      ...
      const { error: err } = await supabase.from('business_logistics_requests').insert({
        ...
        description: [
          ...
          whData.contactCompany ? `Company: ${whData.contactCompany}` : '',
