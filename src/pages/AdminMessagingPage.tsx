...
import {
  ...
  Search, ChevronRight, Clock, CheckCheck, Check, X
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-bold text-gray-900">Private Messages</h1>
            </div>
          </div>
          <button
            onClick={() => setComposing(true)}
            className="flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:from-orange-600 hover:to-red-600 transition-all"
          >
            ...
            {activeThread ? (
              <>
                <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${activeThread.recipient_type === 'agent' ? 'bg-orange-100' : 'bg-blue-100'}`}>
                    {activeThread.recipient_type === 'agent'
                      ? <User className="h-4 w-4 text-orange-600" />
                      : <Building2 className="h-4 w-4 text-blue-600" />}
                  </div>
                  <div>
                    <p className="font-bold text-gray-900 text-sm">{activeThread.recipient_name}</p>
                    <p className="text-xs text-gray-500">{activeThread.subject}</p>
                  </div>
                  <span className={`ml-auto text-xs font-medium px-2.5 py-1 rounded-full ${activeThread.recipient_type === 'agent' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}`}>
                ...
                <div className="px-4 py-3 border-t border-gray-100">
                  <div className="flex items-end gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-3 rounded-xl text-sm font-semibold hover:from-orange-600 hover:to-red-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-gray-400 text-sm">Choose a thread from the left or start a new message</p>
                <button
                  onClick={() => setComposing(true)}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={() => { setComposing(false); setSelectedRecipient(null); setRecipientSearch(''); setRecipients([]); setNewSubject(''); setNewBody(''); setComposeError(''); }}
