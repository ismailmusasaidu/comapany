...
export default function BusinessLoginPage() {
  const { signIn, user, profile, isLoading } = useBusiness();
  const navigate = useNavigate();

  useEffect(() => {
    if (!isLoading && user && profile) navigate('/business/dashboard', { replace: true });
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link to="/auth/forgot-password?portal=business" className="text-xs text-orange-500 hover:text-orange-600 font-medium">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
