...
interface AgentAnalytics {
  ...
  deliveryRate: number;      // delivered / total (excl. cancelled)
...
const SERVICE_COLORS: Record<string, string> = {
  ...
  last_mile: 'bg-red-500',
};

const PACKAGE_COLORS: Record<string, string> = {
  document: 'bg-blue-400',
  parcel: 'bg-orange-400',
  fragile: 'bg-yellow-400',
  heavy: 'bg-slate-500',
};
...
function TrendBadge({ value, suffix = '%' }: { value: { dir: 'up' | 'down' | 'flat'; pct: number }; suffix?: string }) {
  if (value.dir === 'flat') return <span className="flex items-center gap-0.5 text-xs text-gray-400"><Minus className="h-3 w-3" /> 0{suffix}</span>;
  if (value.dir === 'up') return <span className="flex items-center gap-0.5 text-xs text-green-600 font-medium"><ArrowUpRight className="h-3.5 w-3.5" /> {value.pct}{suffix}</span>;
  return <span className="flex items-center gap-0.5 text-xs text-red-500 font-medium"><ArrowDownRight className="h-3.5 w-3.5" /> {value.pct}{suffix}</span>;
}
...
function DonutChart({ segments }: { segments: { label: string; value: number; color: string }[] }) {
  ...
  const colorMap: Record<string, string> = {
    ...
    'bg-green-400': '#4ade80', 'bg-teal-500': '#14b8a6',
  };
  return (
...
export default function AgentDashboardPage() {
  ...
  const fetchData = async () => {
    ...
    const [bookingsRes, requestsRes, recentBRes, recentRRes] = await Promise.all([
      ...
      supabase.from('logistics_requests').select('id, request_ref, title, description, service_type, vehicle_type, origin, destination, quantity, weight_kg, preferred_date, budget_range, status, created_at').eq('agent_id', user.id).order('created_at', { ascending: false }).limit(6),
    ]);
          ...
          <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            {profile.status === 'approved' && (
              <>
                ...
                {/* ─── KPI Row 2: Booking status breakdown ─── */}
                      ...
                      {[
                        ...
                        <div key={row.label} className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <ProgressBar value={row.value} max={row.total || 1} color={row.color} />
                          </div>
                ...
                {/* ─── KPI Row 3: Package & Service breakdowns + requests ─── */}
                <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={row.label} className="flex items-center gap-3">
                          <span className="text-xs text-gray-500 w-20 flex-shrink-0">{row.label}</span>
                          <div className="flex-1">
                            <ProgressBar value={row.value} max={analytics.totalRequests || 1} color={row.color} />
                          </div>
