    ...
    try {
      ...
      const { error: err } = await supabase.from('logistics_requests').insert({
        ...
        description: [
          ...
          ...whData.goods.map((g, i) => `  Item ${i + 1}: ${g.itemName} — ${GOODS_CATEGORIES.find(c => c.value === g.category)?.label || g.category}, Qty: ${g.quantity}${g.weightKg ? ', ' + g.weightKg + ' kg' : ''}${g.fragile ? ' [Fragile]' : ''}${g.highValue ? ' [High Value]' : ''}${g.hazardous ? ' [Hazardous]' : ''}${g.temperatureControlled ? ' [Temp Controlled]' : ''}`),
